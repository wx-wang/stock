 ...
 *
 * 性能优化：breadth API 已有涨跌数据，通过内部 http 复用（不重复调 daily_all）
 * 缓存：5 分钟磁盘
 */
import { getLimitList, getMoneyflowHsgt, getMargin } from '@/server/adapters/tushare'
...
export default defineEventHandler(async () => {
  try {
    ...
    // ── 1. 涨跌数据（复用 breadth API，避免重复拉 5500 条） ──
    ...
    try {
      ...
      // breadth API 不可用，用简单默认
      breadth = 52; limitScore = 55
    }
    ...
    try {
      ...
      if (hsgt.length > 0) {
        ...
        const net = north - south // 万元
        northScore = Math.round(Math.max(0, Math.min(100, 50 + (net / 500000) * 50)))
      }
    } catch {}
    ...
    const result = {
      ...
      components: {
        breadth, limitRatio: limitScore, northFlow: northScore,
        marginChange: marginScore, marketWidth, volumeRatio: volumeScore,
      },
      history,
      _ts: Date.now(),
    }
    ...
    return { success: false, index: 50, label: '数据错误', level: 'neutral', components: {}, history: [] }
  }
})
