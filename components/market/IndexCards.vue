...
async function drawKline() {
  ...
  try {
    ...
    chart.setOption({
      ...
      yAxis: { type: 'value', axisLabel: { fontSize: 10, color: '#6b7280' }, splitLine: { lineStyle: { color: 'rgba(255,255,255,0.04)' } } },
      series: [
        {
          type: 'candlestick',
          data: closes,
        ...
        { type: 'line', data: ma5, symbol: 'none', lineStyle: { color: '#f59e0b', width: 1, opacity: 0.8 }, name: 'MA5' },
...
.index-card {
  ...
  padding: 14px 10px; text-align: center; cursor: pointer; transition: border-color .15s;
}
.index-card:hover { border-color: rgba(255,255,255,0.15); }
...
.is-up .chg-pct { color: #ef4444; }
.is-down .chg-pct { color: #22c55e; }
.idx-amount { font-size: 10px; color: #6b7280; }

.kline-overlay { position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 1000; display: flex; align-items: center; justify-content: center; }
.kline-modal { width: min(900px, 95vw); max-height: 90vh; background: #111827; border-radius: 12px; padding: 20px; }
.kline-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; color: #e5e7eb; font-weight: 600; }
.kline-close { background: none; border: none; color: #9ca3af; font-size: 18px; cursor: pointer; }
